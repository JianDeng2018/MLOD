...
from mlod.core import box_3d_projector, box_3d_encoder
...
def main():
    ...
    for sample_idx in range(dataset.num_samples):
        ...
        # These samples have objects outside of the image, or close to edges
        # if sample_idx in [469, 2979, 3460, 3840, 4569]:
        #     continue

        sys.stdout.write('\r{} / {}'.format(sample_idx, dataset.num_samples))
        ...
        for obj_idx in range(len(gt_labels)):
            ...
            orig_bbox_2d_flipped = flip_box_2d(orig_bbox_2d, image_shape)

            # Do flipping
            start_time = time.time()
            flipped_obj = data_aug.flip_label(obj, image_shape)
            print('flip took', time.time() - start_time)
            ...
            if max_pixel_error > 5:
                print('Large error', sample_idx, max_pixel_error)
