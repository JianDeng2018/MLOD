...
class IntegralImage(object):
    ...
    def _integral_image_3d(self, img):
        ...
        # required so that when accessing coordinate n-1, we get a valid value of 0
        integral_image = np.pad(integral_image, ((1, 0), (1, 0), (1, 0)), 'constant',
                           constant_values=(0))
    ...
    def query(self, cuboids):
        """Input is an array of 3D cuboids 6 coordinates. Each column represents
        a cuboid in the format [x1, y1, z1, x2, y2, z2].T. Thus, the dimensions
        should be 6 x N. The 2 sets of 3D coordinates represent the 2 corners of
        ...
        farthest from the origin. img is the integral image array.
        ...
        if cuboids.shape[0] != 6:
            raise ValueError('Incorrect number of dimensions for query: input dim {}'
                             .format(cuboids.shape[0]))

        if cuboids.shape[1] < 1:
            raise ValueError('The dimension N must be an integer greater than 1: input dim {}'
                             .format(cuboids.shape[1]))
