...
class DepthVgg(img_feature_extractor.ImgFeatureExtractor):
    ...
    def preprocess_input(self, tensor_in, output_size):
        ...
        """
        resized = tf.image.resize_images(tensor_in, output_size)

        # Mean center and normalize
        # ([0, 255] - mean) / 128.0 ~= [-1.0, 1.0]
        rgb_normalized = (resized[..., 0:3] -
                          [self._R_MEAN, self._G_MEAN, self._B_MEAN]) / 128.0

        # Normalize by max depth (70m) and reverse depths
        # (1.0 closest, 0.0 farthest/invalid)
        depth_normalized = 1.0 - (resized[..., 3:] / 70.0)

        input_normalized = tf.concat(
            [rgb_normalized, depth_normalized], axis=3)

        tensor_out = input_normalized
        return tensor_out
    ...
    def build(self,
        ...
        with slim.arg_scope(self.vgg_arg_scope(
            ...
            with tf.variable_scope(scope, 'depth_vgg', [inputs]) as sc:
