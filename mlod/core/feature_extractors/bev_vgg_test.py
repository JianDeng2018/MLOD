...
from mlod.core.feature_extractors import bev_vgg as vgg
from mlod.datasets.kitti.kitti_dataset import KittiDataset
from mlod.protos import train_pb2
...
class BevVggTest(tf.test.TestCase):
    ...
    def test_vgg_layers_build(self):
        ...
        total_clusters = np.sum(self.dataset.num_clusters)
        num_bev_maps = total_clusters + 1  # Height Maps + Density Map

        with tf.Graph().as_default():
            with tf.name_scope('input'):
                # BEV image placeholder
                image_placeholder = tf.placeholder(
                    tf.float32, (None, 700, 800, num_bev_maps))
