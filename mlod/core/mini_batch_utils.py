...
from mlod.core.mini_batch_preprocessor import MiniBatchPreprocessor
from mlod.core.minibatch_samplers import balanced_positive_negative_sampler
from mlod.core.minibatch_samplers import balanced_positive_negative_sampler2
...
class MiniBatchUtils:
    ...
    def sample_mlod_mini_batch(self, anchor_ious):
        """ Samples a mini batch to train MLOD with preconfigured
        ...
        sampler = self._mini_batch_sampler
        return self.sample_mini_batch(anchor_ious,
                                      self.mlod_mini_batch_size,
                                      self.mlod_neg_iou_range,
                                      self.mlod_pos_iou_range,
                                      sampler)

    def double_sample_img_mini_batch(self, anchor_ious, masks):

        negative_iou_range = self.mlod_img_neg_iou_range
        positive_iou_range = self.mlod_img_pos_iou_range

        mb_ious = tf.boolean_mask(anchor_ious,masks)

        bkg_and_neg_labels = tf.less(mb_ious, negative_iou_range[1])
        pos_labels = tf.greater(mb_ious, positive_iou_range[0])
        indicator = tf.logical_or(pos_labels, bkg_and_neg_labels)

        masks_idx = tf.where(masks)
        full_mask_idx = tf.boolean_mask(masks_idx,indicator)
        pos_mask_idx = tf.boolean_mask(masks_idx,pos_labels)

        full_masks = ops.indices_to_dense_vector(full_mask_idx,
                                               tf.shape(masks)[0])

        pos_masks = ops.indices_to_dense_vector(pos_mask_idx,
                                               tf.shape(masks)[0])

        full_masks = tf.equal(full_masks,1)
        pos_masks = tf.equal(pos_masks,1)

        return full_masks, pos_masks

    def sample_mlod_img_mini_batch(self, anchor_ious):
        """ Samples a mini batch to train MLOD image channel with preconfigured
            mini batch size and 2D iou ranges
        ...
        sampler = self._mini_batch_sampler
        return self.sample_mini_batch(anchor_ious,
                                      self.mlod_img_mini_batch_size,
                                      self.mlod_img_neg_iou_range,
                                      self.mlod_img_pos_iou_range,
                                      sampler)
    ...
    def _parse_anchors_info(self, anchors_info):
        ...
        image_ious = np.asarray(
            anchors_info[:, self.col_img_ious], dtype=np.float32)        

        image_offsets = np.asarray(
            anchors_info[:, self.col_img_offsets_lo:self.col_img_offsets_hi],
            dtype=np.float32)

        image_classes = np.asarray(
            anchors_info[:, self.col_img_class_idx], dtype=np.float32)

        return anchor_indices, anchor_ious, \
            anchor_offsets, anchor_classes, \
            image_ious, image_offsets, image_classes
