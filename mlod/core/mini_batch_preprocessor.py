...
from mlod.core import box_3d_encoder, anchor_projector
...
class MiniBatchPreprocessor(object):
    def __init__(self,
        ...
        self.calib_dir = self._dataset.calib_dir
    ...
    def _calculate_anchors_info(self,
        ...
        anchors_on_img, _ = np.asarray(anchor_projector.project_to_image_space(
                        anchors, calib_p2, image_shape))

        gt_img_boxes_for_2d_iou = np.asarray([[gt_obj.x1, gt_obj.y1, gt_obj.x2, gt_obj.y2] for gt_obj in gt_labels])
        ...
        for gt_idx in range(len(gt_labels)):
            ...
            img_offsets = anchor_encoder.img_box_to_offset(anchors_on_img_to_update, gt_img_box_for_2d_iou)
            ...
            all_info[update_img_indices,
                    self.mini_batch_utils.col_img_offsets_lo:
                    self.mini_batch_utils.col_img_offsets_hi] = img_offsets
            all_info[update_img_indices,
    ...
    def preprocess(self, indices):
        ...
        for sample_idx in indices:
            ...
            anchors_info = self._calculate_anchors_info(
                all_anchor_boxes_3d, empty_anchor_filter, filtered_gt_list, calib_p2, image_shape)
