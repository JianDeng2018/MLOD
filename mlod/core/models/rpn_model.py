...
from mlod.builders import feature_extractor_builder
...
class RpnModel(model.DetectionModel):
    ...
    PL_IMAGE_IOUS = 'image_ious_pl'
    PL_IMAGE_OFFSETS = 'image_offsets_pl'
    PL_IMAGE_CLASSES = 'image_classes_pl'
    ...
    PRED_MB_IMG_OBJECTNESS_GT = 'rpn_mb_img_objectness_gt'
    PRED_MB_IMG_OFFSETS_GT = 'rpn_mb_img_offsets_gt'

    PRED_MB_MASK = 'rpn_mb_mask'
    PRED_MB_OBJECTNESS = 'rpn_mb_objectness'
    PRED_SUB_MB_OBJECTNESS = 'rpn_mb_sub_objectness'
    ...
    PRED_DEPTH_MAP = 'processed_depth_map'
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        self.multi_check = rpn_config.multi_check
        self.cls_reg_separated = rpn_config.cls_reg_separated
        self.image_idx = self._config.image_channel_indices
        ...
        if self.lidar_only:
            ...
            for i in range(self.num_views):
                self._img_feature_extractor.append(feature_extractor_builder.get_extractor(
        ...
        self._BEV_freeze_prob = self._config.BEV_freeze_probability
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('img_input'):
            ...
            if self.lidar_only:
                img_input_placeholder = self._add_placeholder(
                    tf.float32,
                    [None,None, None, self._img_depth],
                    self.PL_IMG_INPUT)
                self._img_input_batches = img_input_placeholder
            else:
                img_input_placeholder = self._add_placeholder(
                    tf.float32,
                ...
                self._img_input_batches = tf.expand_dims(img_input_placeholder,0)
            self.img_input_placeholder = img_input_placeholder

            if self.lidar_only and self.num_views > 0:
                self._img_preprocessed = tf.image.resize_images(self._img_input_batches, self._img_pixel_size)
            elif not self.lidar_only:
            ...
            # Summary Image
            if self.lidar_only and self.num_views > 0:
                if self.dataset.config.add_depth:
                    rgb_image = self._img_preprocessed[..., 0:3]
                    depth_map = self._img_preprocessed[..., 3:]

                    tf.summary.image("rgb_image", rgb_image,
                                     max_outputs=2)
        ...
        with tf.variable_scope('depth_input'):
            depth_input_placeholder = self._add_placeholder(tf.float32, [None,None,None],
                                                          self.PL_DPT_INPUT)
            #preprocess depth map
            self._depth_input_batches = tf.expand_dims(depth_input_placeholder,axis=-1)
            #must use nearest neighbor method
            self._depth_preprocessed = tf.image.resize_images(self._depth_input_batches, self._img_pixel_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)

            tf.summary.image("depth_image", self._depth_preprocessed, max_outputs=2)
        ...
        with tf.variable_scope('pl_anchors'):
            ...
            self._add_placeholder(tf.float32, [None],
                                  self.PL_IMAGE_IOUS)
            self._add_placeholder(tf.float32, [None, 4],
                                  self.PL_IMAGE_OFFSETS)
            self._add_placeholder(tf.float32, [None],
                                  self.PL_IMAGE_CLASSES)
            ...
            with tf.variable_scope('sample_info'):
                ...
                if self.lidar_only:
                    self._add_placeholder(
                        tf.float32, [None,3, 4], self.PL_CALIB_P2)
                else:
                    self._add_placeholder(
    ...
    def _set_up_feature_extractors(self):
        ...
        if self.lidar_only:
            ...
            self.img_end_points = []
            for i in range(self.num_views):
                local_img_preprocessed = tf.expand_dims(self._img_preprocessed[i,:,:,:],axis=0)
                local_img_feature_maps, local_img_end_points = \
                self._img_feature_extractor[i].build(
                    local_img_preprocessed,
                    self._img_pixel_size,
                    self._is_training,
                    'img_vgg_pyr'+str(i))
                self.img_feature_maps.append(local_img_feature_maps)
                self.img_end_points.append(local_img_end_points)
        else:
    ...
    def build(self):
        ...
        if not (self._path_drop_probabilities[0] ==
            ...
            with tf.variable_scope('rpn_path_drop'):
                ...
                if self.lidar_only:
                    for i in range(self.num_views):
                        img_proposal_input[i] = tf.multiply(img_proposal_input[i],
                                                         img_mask)
                else:
                ...
                fusion_mean_div_factor = img_mask*self.num_views + bev_mask

        # BEV branch freeze
        if not (self._BEV_freeze_prob == 0.0):
            random_values = tf.random_uniform(shape=[1],
                                              minval=0.0,
                                              maxval=1.0)
            freeze = lambda: tf.constant(0.0)
            not_freeze = lambda: tf.constant(1.0)
            self.bev_freeze_mask = tf.case([(tf.greater(random_values[0], self._BEV_freeze_prob), not_freeze)],default=freeze)
        else:
            self.bev_freeze_mask = tf.constant(1.0)
        ...
        if self.multi_check:
            objectness,offsets,sub_cls_objectness = self.multi_check_anchor_predictor([bev_proposal_rois])
        else:
            # sub_cls_objectness is empty
            objectness,offsets,sub_cls_objectness = self.anchor_predictor(bev_proposal_rois, img_proposal_rois, fusion_mean_div_factor)
        ...
        with tf.variable_scope('proposals'):
            ...
            with tf.variable_scope('softmax'):
                ...
                sub_obj_softmax_list = []
                for sub_objectness in sub_cls_objectness:
                    sub_objectness_softmax = tf.nn.softmax(sub_objectness)
                    sub_obj_softmax_list.append(sub_objectness_softmax)
        ...
        img_ious_gt = self.placeholders[self.PL_IMAGE_IOUS]
        img_offsets_gt = self.placeholders[self.PL_IMAGE_OFFSETS]
        img_classes_gt = self.placeholders[self.PL_IMAGE_CLASSES]
        ...
        with tf.variable_scope('one_hot_classes'):
            ...
            min_img_pos_iou = \
                self.dataset.kitti_utils.mini_batch_utils.rpn_img_pos_iou_range[0]
            objectness_img_classes_gt = tf.cast(
                tf.greater_equal(img_ious_gt, min_img_pos_iou),
                dtype=tf.int32)
            objectness_img_gt = tf.one_hot(
                objectness_img_classes_gt, depth=2,
                on_value=1.0 - self._config.label_smoothing_epsilon,
                off_value=self._config.label_smoothing_epsilon)
        ...
        with tf.variable_scope('prediction_mini_batch'):
            ...
            sub_obj_masked_list = []
            for sub_objectness in sub_cls_objectness:
                sub_objectness_masked = tf.boolean_mask(sub_objectness, mini_batch_mask)
                sub_obj_masked_list.append(sub_objectness_masked)
        ...
        with tf.variable_scope('ground_truth_mini_batch'):
            ...
            objectness_img_gt_masked = tf.boolean_mask(
                objectness_img_gt, mini_batch_mask)
            offsets_img_gt_masked = tf.boolean_mask(img_offsets_gt,
                                                mini_batch_mask)
        ...
        if self._train_val_test in ['train', 'val']:
            # All anchors
            #predictions['img_feature'] = self.img_feature_maps
            #predictions['bev_feature'] = self.bev_feature_maps
            ...
            predictions[self.PRED_SUB_MB_OBJECTNESS] = sub_obj_masked_list
            ...
            predictions[self.PRED_MB_IMG_OFFSETS_GT] = offsets_img_gt_masked
            predictions[self.PRED_MB_IMG_OBJECTNESS_GT] = objectness_img_gt_masked
            ...
            predictions[self.PRED_DEPTH_MAP] = self._depth_preprocessed
    ...
    def anchor_predictor(self,bev_proposal_rois, img_proposal_rois,fusion_mean_div_factor):

        with tf.variable_scope('proposal_roi_fusion'):
            rpn_fusion_out = None
            if self.lidar_only and self.num_views ==0:
                rpn_fusion_out = bev_proposal_rois
            elif self._fusion_method == 'mean':
                if self.lidar_only:
                    img_proposal_rois.append(bev_proposal_rois)
                    tf_features_concat = tf.stack(img_proposal_rois,axis=0)
                    tf_features_sum = tf.reduce_sum(tf_features_concat,axis=0)
                else:
                ...
                if self.lidar_only:
                    img_proposal_rois.append(bev_proposal_rois)
                    tf_features_concat = tf.stack(img_proposal_rois,axis=3)
                else:
        ...
        with tf.variable_scope('histograms_feature_extractor'):
            ...
            with tf.variable_scope('img_vgg'):
                if self.lidar_only:
                    for i in range(self.num_views):
                        img_end_pts = self.img_end_points[i]
                        for end_point in img_end_pts:
                            tf.summary.histogram(
                                end_point, img_end_pts[end_point])
                else:
        ...
        return objectness,offsets, sub_cls_objectness

    def multi_check_anchor_predictor(self,proposal_rois_list):

        # TODO: move this section into an separate AnchorPredictor class
        with tf.variable_scope('anchor_predictor', 'ap'):
            # Parse rpn layers config
            layers_config = self._config.layers_config.rpn_config
            
            # bev_rois, img_rois
            cls_weight_list = layers_config.cls_input_weights
            reg_weight_list = layers_config.reg_input_weights
            l2_weight_decay = layers_config.l2_weight_decay
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                reg_fc_list = []
                for branch_idx in range(len(proposal_rois_list)):
                    # Use conv2d instead of fully_connected layers.
                    cls_fc6 = slim.conv2d(proposal_rois_list[branch_idx],
                                          layers_config.cls_fc6,
                                          self._proposal_roi_crop_size,
                                          padding='VALID',
                                          scope='cls_fc6_'+str(branch_idx))

                    cls_fc6_drop = slim.dropout(cls_fc6,
                                            layers_config.keep_prob,
                                            is_training=self._is_training,
                                            scope='cls_fc6_drop_'+str(branch_idx))

                    cls_fc7 = slim.conv2d(cls_fc6_drop,
                                          layers_config.cls_fc7,
                                          [1, 1],
                                            scope='cls_fc7_'+str(branch_idx))

                    cls_fc_list.append(cls_fc7)

                    #obtain the sub classification objectness
                    sub_cls_fc7_drop = slim.dropout(cls_fc7,
                                                layers_config.keep_prob,
                                                is_training=self._is_training,
                                                scope='cls_fc7_drop_'+str(branch_idx))

                    sub_cls_fc8 = slim.conv2d(sub_cls_fc7_drop,
                                          2,
                                          [1, 1],
                                          activation_fn=None,
                                          scope='cls_fc8_'+str(branch_idx))

                    sub_objectness = tf.squeeze(
                            sub_cls_fc8, [1, 2],
                            name='cls_fc8/squeezed_'+str(branch_idx))

                    sub_cls_objectness.append(sub_objectness)

                # fusion classification layer
                fusion_cls_fc = cls_fc_list[0]

                cls_fc7_drop = slim.dropout(fusion_cls_fc,
                                                layers_config.keep_prob,
                                                is_training=self._is_training,
                                                scope='cls_fc7_drop')
                ...
                cls_fc8_drop = slim.dropout(cls_fc8,
                                                layers_config.keep_prob,
                                                is_training=self._is_training,
                                                scope='cls_fc8_drop')

                cls_fc9 = slim.conv2d(cls_fc8_drop,
                                      2,
                                      [1, 1],
                                      activation_fn=None,
                                      scope='cls_fc9')
                ...
                objectness = tf.squeeze(
                        cls_fc9, [1, 2],
                        name='cls_fc9/squeezed')

                #if self.cls_reg_separated:
                if True:
                    for branch_idx in range(len(proposal_rois_list)):
                        # Use conv2d instead of fully_connected layers.
                        reg_fc6 = slim.conv2d(proposal_rois_list[branch_idx],
                                              layers_config.reg_fc6,
                                              self._proposal_roi_crop_size,
                                              padding='VALID',
                                              scope='reg_fc6_'+str(branch_idx))

                        reg_fc6_drop = slim.dropout(reg_fc6,
                                                    layers_config.keep_prob,
                                                    is_training=self._is_training,
                                                    scope='reg_fc6_drop_'+str(branch_idx))

                        reg_fc7 = slim.conv2d(reg_fc6_drop,
                                              layers_config.reg_fc7,
                                              [1, 1],
                                              scope='reg_fc7_'+str(branch_idx))

                        reg_fc_list.append(reg_fc7)
                    
                else:
                    reg_fc_list =cls_fc_list
                    
                #fusion regression layer
                weighted_reg_fc_list = [a*b for a,b in zip(reg_fc_list,reg_weight_list)]
                fusion_reg_fc = tf.concat(weighted_reg_fc_list,axis=-1)
                

                reg_fc7_drop = slim.dropout(fusion_reg_fc,
                                            layers_config.keep_prob,
                                            is_training=self._is_training,
                                            scope='reg_fc7_drop_'+str(branch_idx))

                reg_fc8 = slim.conv2d(reg_fc7_drop,
                                      6,
                                      [1, 1],
                                      activation_fn=None,
                                      scope='reg_fc8_'+str(branch_idx))
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        depth_input = sample.get(constants.KEY_DPT_INPUT)

        if self.lidar_only:
            # Image shape (None,h, w)
            image_shape = [image_input.shape[1], image_input.shape[2]]
        else:
            # Image shape (h, w)
        ...
        self._placeholder_inputs[self.PL_DPT_INPUT] = depth_input
    ...
    def _fill_anchor_pl_inputs(self,
        ...
        if self._train_val_test in ['train', 'val']:
            ...
            if anchors_info:
                anchor_indices, anchors_ious, anchor_offsets, \
                    anchor_classes, image_ious, image_offsets, \
                    image_classes = anchors_info
        ...
        image_ious = np.asarray(image_ious)
        image_offsets = np.asarray(image_offsets)
        ...
        if self._train_val_test in ['train', 'val'] and \
            ...
            self._placeholder_inputs[self.PL_IMAGE_IOUS] = image_ious
            self._placeholder_inputs[self.PL_IMAGE_OFFSETS] = image_offsets
            self._placeholder_inputs[self.PL_IMAGE_CLASSES] = image_classes
        ...
        # During test, or val when there is no anchor info
            ...
            # During testing, or validation with no gt, fill these in with 0s
            self._placeholder_inputs[self.PL_IMAGE_IOUS] = \
                np.zeros(num_anchors)
            self._placeholder_inputs[self.PL_IMAGE_OFFSETS] = \
                np.zeros([num_anchors, 4])
            self._placeholder_inputs[self.PL_IMAGE_CLASSES] = \
                np.zeros(num_anchors)
    ...
    def loss(self, prediction_dict):
        ...
        objectness_img_gt = prediction_dict[self.PRED_MB_IMG_OBJECTNESS_GT]
        offsets_gt = prediction_dict[self.PRED_MB_OFFSETS_GT]
        img_offsets_gt = prediction_dict[self.PRED_MB_IMG_OFFSETS_GT]

        # Predictions
        with tf.variable_scope('rpn_prediction_mini_batch'):
            objectness = prediction_dict[self.PRED_MB_OBJECTNESS]
            sub_obj_masked_list = prediction_dict[self.PRED_SUB_MB_OBJECTNESS]
        ...
        with tf.variable_scope('rpn_losses'):
            with tf.variable_scope('objectness'):
                ...
                sub_cls_loss_weight = self._config.loss_config.sub_cls_loss_weight
                bev_freeze_mask = self.bev_freeze_mask
